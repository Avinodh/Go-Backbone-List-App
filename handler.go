...
func SearchHackathon(rw http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	query_string := vars["query"]
	...
	rows, err := db.Query("SELECT * FROM Hackathons WHERE name LIKE $1 || '%'", query_string)
	checkErr(err)
